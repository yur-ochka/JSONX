...
describe("JSONX Integration Tests", () => {
  ...
  test("date formatting transformation", async () => {
    const template = {
      templates: [
        {
          name: "event",
          output: {
            title: "$.title",
            date: { expr: "formatDate($.timestamp, 'YYYY-MM-DD')" },
            time: { expr: "formatDate($.timestamp, 'hh:mm')" },
            isFuture: {
              expr: "greaterThan($.timestamp, now())", // Use a function
            },
          },
        },
      ],
      root: {
        events: { apply: "event", from: "$.events" },
      },
    };
    ...
    expect(result.events[1].date).toBe("2024-12-01");
  });
});
